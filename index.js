...
const StartGameHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      .speak(speechResponse).reprompt(speechResponse).getResponse();
  }
};
...
const GuessALetterHandler = {
  ...
  handle(handlerInput) {
    ...
    if (!attributes.game_answer)
      return handlerInput.responseBuilder
        .speak("Start another game.").reprompt().getResponse();

    let answer = attributes.game_answer;
    ...
    if (positions.length === 0) {
      ...
      speechResponse = "You guessed it, the position is: " + positions[0];
    } else {
